...
export default function ExampleFileDialog() {
  const {
    state,
    actions: {setShowExampleFileDialog, setIsLoading},
  } = useUi();
  const {
    state: viewerState,
  ...
  const handleLoadExample = async (fileSetting, cityDatasetKey) => {
    ...
    setIsLoading(false);
    setShowExampleFileDialog(false);
  };

  return (
    <Transition.Root show={state.showExampleFileDialog} as={Fragment}>
      <Dialog
        as="div"
        className="relative z-30"
        onClose={() => setShowExampleFileDialog(false)}
      >
        <Transition.Child
          as={Fragment}
              ...
              <Dialog.Panel className="md:max-w-7xl md:mx-10 relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-sm sm:p-6">
                <div className="absolute top-0 right-0 hidden pt-4 pr-4 sm:block">
                  <button
                    ...
                    onClick={() => setShowExampleFileDialog(false)}
                  >
