...
import SelectTool from './create-flow-1-select-tool';
import ToolInput from './create-flow-2-tool-input';
import RunTool from './create-flow-3-run-tool';

function getTool(data: any, toolName: string) {
  const taskModule = data.find(
    (taskModule: TaskModule) => taskModule.task_id === toolName
  );
  return taskModule?.config?.tools[0];
}
...
export type TaskModuleTool = {
  ...
  parameters: TaskModuleToolParameter[];
};

export type TaskModuleToolParameter = {
  name: string;
  description: string;
  required: boolean;
  type: string;
...
export default function CreateFlowDialog() {
  ...
  const setSelectedStep = (stepId: any) => {
  ...
  const handleSelectTool = (toolName: any, commandName: any) => {
                ...
                {data && (
                  <>
                    ...
                    {selectedStep.id === 'step-1' ? (
                      ...
                      <ToolInput
                        taskModuleTool={getTool(data, selectedToolName)}
