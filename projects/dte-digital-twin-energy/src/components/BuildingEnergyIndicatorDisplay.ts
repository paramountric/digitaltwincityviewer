...
@customElement('dte-building-energy-indicator-display')
export class BuildingEnergyIndicatorDisplay extends MobxLitElement {
  static styles = css`
    :host table {
      margin-bottom: 5px;
    }
  ...
  getIndicatorYears(key: string) {
    const y2020 = this.properties[`${key}2020M2`] || 0;
    const y2030 = this.properties[`${key}2030M2`] || 0;
    const y2050 = this.properties[`${key}2050M2`] || 0;
  ...
  render(): TemplateResult {
    ...
    return html`<div>
      ...
      <table>
        <tbody>
          ${properties.map(key => {
            const val = this.formatValue(this.properties, key);
            return html`<tr>
