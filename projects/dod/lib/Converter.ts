...
export class ObjectWrapper {
  ...
  geometryType;
  extras;
  constructor(bufferGeometry: any, meta: any, geometryType: any, extras?: any) {
...
export default class Converter {
  ...
  private directConverterExists(obj: any) {
  ...
  private convertToGeometryData(
    obj: any,
    scale = true
  ): Promise<GeometryData> | unknown {
    return this.GeometryDataConverterMapping[this.getSpeckleType(obj)](
      obj,
      scale
    );
  }
   ...
   * @return {[type]}     [description]
   */
  private async convert(obj: any, scale = true) {
    if (obj.referencedId) obj = await this.resolveReference(obj);
    try {
      if (this.directConverterExists(obj)) {
      ...
      console.warn(
        `(Direct convert) Failed to convert object with id: ${obj.id}`
      );
      throw e;
    }
  }
   ...
   * BLOCK INSTANCE
   */
  private async BlockInstanceToBufferGeometry(obj: any) {
    const cF = 1;
    ...
    console.log('todo: this is block instance - fix matrices');
  ...
  private async PointcloudToBufferGeometry(obj: any, scale = true) {
    console.log('todo: point cloud is not supported');
    return new ObjectWrapper(obj, obj, 'pointcloud');
  }
  ...
  private async BrepToBufferGeometry(obj: any, scale = true) {
    try {
      ...
      return new ObjectWrapper(bufferGeometry, obj, 'brep');
    } catch (e) {
   ...
   * MESH
   */
  private async MeshToGeometryData(
    obj: any,
    scale = true
  ...
  private async MeshToBufferGeometry(obj: any, scale = true) {
    try {
      const geo = await this.MeshToGeometryData(obj, scale);
      return new ObjectWrapper(geo, obj, 'mesh');
    } catch (e) {
   ...
   * POINT
   */
  private async PointToGeometryData(
    obj: any,
    scale = true
  ...
  private async PointToBufferGeometry(obj: any, scale = true) {
    const geo = await this.PointToGeometryData(obj, scale);
    return new ObjectWrapper(geo, obj, 'point');
  }
  ...
  private async LineToBufferGeometry(obj: any, scale = true) {
    ...
    const geo = await this.LineToGeometryData(obj, scale);
    return new ObjectWrapper(geo, obj, 'line');
  }
  ...
  async PolylineToBufferGeometry(obj: any, scale = true) {
    const geometry = await this.PolylineToGeometryData(obj, scale);
  ...
  //   const geometryData: GeometryData = await this.PolycurveToGeometryData(
  //     object,
  //     scale
  //   );
  ...
  //   const T = new Matrix4().setPosition(
  //     origin.multiplyScalar(conversionFactor)
  //   );
  ...
  //   const geometry = Geometry.makeLineGeometry(
  //     await this.ArcToGeometryData(obj, scale)
  //   );
  ...
  PointToVector3(obj: any, scale = true) {
    const conversionFactor = 1;
    let v = null;
    if (obj.value) {
