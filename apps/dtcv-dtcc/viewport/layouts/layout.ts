...
export abstract class Layout {
  ...
  constructor({ parentFeature, viewport }: LayoutProps) {
    ...
    if (
      ...
      this.cameraFrame = {
        ...
        target: parentFeature.properties?._target || [0, 0, 0],
        rotationOrbit: parentFeature.properties?._rotationOrbit || 0,
        rotationX: parentFeature.properties?._rotationX || 0,
  ...
  applyFeatures(featureList: Feature[]) {
    ...
    this.featureMap = new Map(
      featureList.map((f) => [f.versionUri || getVersionUri(f), f])
    );
  ...
  getViewState({ view3d }: GetViewStateProps): any {
    const frame = {
      ...this.cameraFrame,
    };
    return frame;
