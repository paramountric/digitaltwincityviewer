...
import { MVTLayerProps, TerrainLayerProps, Tile3DLayerProps } from '@deck.gl/geo-layers';
import { Feature, FeatureMap, FeatureProperties, getVersionUri } from '../feature';
...
export abstract class Layout {
  ...
  constructor({ parentFeature, viewport }: LayoutProps) {
    ...
    this.featureAnimations = new Map();
    this.updateTriggers = {};
    this.featureMap = new Map();

    if (parentFeature.properties?._longitude && parentFeature.properties?._latitude) {
      this.cameraFrame = {
        viewX: 0,
        viewY: 0,
  ...
  getActiveFrame() {
    ...
    // const activeFrame = cameraFramesReversed.find((f) => f.timing <= animationCursor);
    // return activeFrame;
  }
  ...
  getFrameTiming(versionUri: string): number | null {
    ...
    const frameIndex = this.cameraAnimation.keyframes.findIndex((k) => k.versionUri === versionUri);
    if (frameIndex === -1) {
      return null;
    }
  ...
  getCameraFrameByVersionUri(versionUri: string) {
    ...
    const frame = this.cameraAnimation.keyframes.find((k) => k.versionUri === versionUri);
    return frame;
  }
  ...
  getVisibleFeatures(atTiming?: number): Feature[] {
    ...
    // this is general
    const visibleFeatures = [...this.featureMap.values()].filter((f) => !f.properties?._hide);
