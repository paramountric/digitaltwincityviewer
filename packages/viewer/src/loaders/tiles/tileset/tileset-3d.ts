...
import {
  ...
  getFrameState,
  limitSelectedTiles,
} from './helpers/frame-state.js';
import {
  getZoomFromBoundingVolume,
  getZoomFromExtent,
  getZoomFromFullExtent,
} from './helpers/zoom.js';
...
export default class Tileset3D {
  ...
  hasExtension(extensionName: string): boolean {
    return Boolean(
      this._extensionsUsed && this._extensionsUsed.indexOf(extensionName) > -1
    );
  }
  ...
  private doUpdate(viewports: Viewport[] | Viewport): void {
    ...
    const preparedViewports =
      viewports instanceof Array ? viewports : [viewports];
  ...
  _onTraversalEnd(frameState: FrameState): void {
    ...
    currentFrameStateData._requestedTiles = Object.values(
      this._traverser.requestedTiles
    );
    currentFrameStateData._emptyTiles = Object.values(
      this._traverser.emptyTiles
    );
  ...
  _updateTiles(): void {
    ...
    for (const frameStateKey in this.frameStateData) {
      ...
      this.selectedTiles = this.selectedTiles.concat(
        frameStateDataValue.selectedTiles
      );
      this._requestedTiles = this._requestedTiles.concat(
        frameStateDataValue._requestedTiles
      );
      this._emptyTiles = this._emptyTiles.concat(
        frameStateDataValue._emptyTiles
      );
    }
  ...
  private calculateViewPropsI3S() {
    ...
    if (fullExtent) {
      ...
      this.zoom = getZoomFromFullExtent(
        fullExtent,
        this.cartographicCenter,
  ...
  private calculateViewPropsTiles3D() {
    ...
    if (center[0] !== 0 || center[1] !== 0 || center[2] !== 0) {
      this.cartographicCenter = Ellipsoid.WGS84.cartesianToCartographic(
        center,
        new Vector3()
      );
    } else {
    ...
    this.zoom = getZoomFromBoundingVolume(
      root.boundingVolume,
      this.cartographicCenter
    );
  }
  ...
  _onTileLoad(tile, loaded) {
    ...
    if (this.type === TILESET_TYPE.I3S) {
      ...
      const nodesInNodePages =
        this.tileset?.nodePagesTile?.nodesInNodePages || 0;
  ...
  _unloadTile(tile) {
    this.gpuMemoryUsageInBytes -=
      (tile.content && tile.content.byteLength) || 0;
  ...
  _initializeTiles3DTileset(tilesetJson) {
    ...
    if (
      ...
      throw new Error(
        'The tileset must be 3D Tiles version 0.0 or 1.0. Or actually 1.1'
      );
    }
