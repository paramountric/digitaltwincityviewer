...
export default class TileHeader {
  ...
  get contentReady() {
    return (
      this.contentState === TILE_CONTENT_STATE.READY || this.hasEmptyContent
    );
  }
  ...
  _getPriority() {
     ...
     */
    const maySkipTile =
      this.refine === TILE_REFINEMENT.ADD || skipLevelOfDetail;
    ...
    const rootScreenSpaceError = traverser.root
      ? traverser.root._screenSpaceError
      : 0.0;
  ...
  distanceToTile(frameState: FrameState): number {
    ...
    return Math.sqrt(
      Math.max(boundingVolume.distanceSquaredTo(frameState.camera.position), 0)
    );
  }
  ...
  insideViewerRequestVolume(frameState: FrameState) {
    ...
    return (
      !viewerRequestVolume ||
      viewerRequestVolume.distanceSquaredTo(frameState.camera.position) <= 0
    );
  }
  ...
  _initializeTransforms(tileHeader) {
    ...
    this.transform = tileHeader.transform
      ? new Matrix4(tileHeader.transform)
      : new Matrix4();
    ...
    this.computedTransform = new Matrix4(parentTransform).multiplyRight(
      this.transform
    );

    const parentInitialTransform =
      parent && parent._initialTransform
        ? parent._initialTransform.clone()
        : new Matrix4();
    this._initialTransform = new Matrix4(parentInitialTransform).multiplyRight(
      this.transform
    );
  }
  ...
  _getRefine(refine) {
    ...
    return (
      refine || (this.parent && this.parent.refine) || TILE_REFINEMENT.REPLACE
    );
  }
  ...
  _updateTransform(parentTransform = new Matrix4()) {
    const computedTransform = parentTransform
      .clone()
      .multiplyRight(this.transform);
    const didTransformChange = !computedTransform.equals(
      this.computedTransform
    );
