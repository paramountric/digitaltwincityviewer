...
import { textMatrixToTexture } from './utils.js';

import fragmentShader from './axes-fragment.glsl.js';
import gridVertex from './grid-vertex.glsl.js';
import labelVertex from './label-vertex.glsl.js';
import labelFragment from './label-fragment.glsl.js';
...
export default class AxesLayer extends Layer {
  ...
  initializeState() {
    ...
    attributeManager.addInstanced({
      ...
      instanceIsTitle: {
        size: 1,
        update: this.calculateInstanceIsTitle,
        noAlloc: true,
      },
    });
  ...
  draw({ uniforms }) {
    const { gridDims, gridCenter, modelsByName, labelTexture, numInstances } =
      this.state;
    ...
    if (labelTexture) {
      ...
      modelsByName.grids
        .setUniforms(Object.assign({}, uniforms, baseUniforms))
        .draw();
      modelsByName.labels
  ...
  _getModels(gl) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      for (let j = 0; j < 4; j++) {
        labelPositions = labelPositions.concat(
          gridPositions.slice(i * 3, i * 3 + 3)
        );
  ...
  renderLabelTexture(ticks) {
    ...
    const textureInfo = textMatrixToTexture(
      this.context.gl,
      ticks,
      DEFAULT_FONT_SIZE
    );
